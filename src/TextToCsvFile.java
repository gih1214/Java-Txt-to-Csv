...
public class TextToCsvFile implements TextToCsvFileAble {
    private static final String WORKSPACE = "C:\\green_workspace\\practice_lab\\zipcode-search\\";
    private static final String RESOURCE = WORKSPACE + "resource\\";

    // path = 내 프로젝트 경로와 내부 resource 폴더가 내장되어 있습니다.
    // encType = UTF-8, EUC-KR 등을 입력하면 됩니다.
    protected String readFile(String path, String encType) { // 메서드화
        try {
    ...
    protected int writeFile(String path, String encType, String csvStr) { // 메서드화
        try {
            FileWriter writer = new FileWriter(new File(RESOURCE + path), Charset.forName(encType));
